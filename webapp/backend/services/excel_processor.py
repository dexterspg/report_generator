...
class ExcelProcessor:
    ...
    def process_file(self, 
        ...
        try:
            ...
            required_columns = [
                'Transaction Type', 'Fiscal Year', 'Fiscal Period', 'Activation Group ID',
                'Contract Name', 'Vendor', 'Account Number', 'Contract Currency',
                'Amount in Contract Currency', 'Company Currency Exchange Rate',
            ...
            # credito_convertido_letter = get_column_letter(credito_convertido_idx + 1)
             ...
             #Divide the template into multiple sheets based on the input_df column Transaction Type
            document_types = input_df['Transaction Type'].unique().tolist()
            # Create workbook
            ...
            # Add data rows
            for transaction_type, group_data in input_df.groupby('Transaction Type'):
                template_ws = sheet_dict[transaction_type]
    ...
    def process_file_all_company_codes(self,
                                        source_file_path: str,
                                        output_dir: str,
                                        input_header_start: int = 27,
                                        input_data_start: int = 28,
        ...
        """
        Process the Excel file and generate separate files for each unique company code.

        Args:
            source_file_path: Path to input Excel file
            output_dir: Directory path for output Excel files
            input_header_start: Row number where headers start in input (1-indexed)
            input_data_start: Row number where data starts in input (1-indexed)
            template_header_start: Row number where headers start in output (1-indexed)
            template_data_start: Row number where data starts in output (1-indexed)
        ...
        Returns:
            Dictionary with processing results including list of generated files
        """
        try:
            # Read input Excel file
            input_df = pd.read_excel(source_file_path, header=input_header_start - 1)
            ...
            # Check for Company Code column
            if 'Company Code' not in input_df.columns:
                raise ValueError("Company Code column not found in the input file")

            # Get unique company codes
            unique_company_codes = input_df['Company Code'].dropna().unique().tolist()
            unique_company_codes = sorted([str(code) for code in unique_company_codes])

            if len(unique_company_codes) == 0:
                raise ValueError("No company codes found in the input file")

            print(f"Found {len(unique_company_codes)} unique company codes: {unique_company_codes}")
            ...
            for company_code in unique_company_codes:
                print(f"Processing company code: {company_code}")

                # Filter data for this company code
                company_df = input_df[input_df['Company Code'].astype(str) == str(company_code)].copy()
                company_df = company_df.reset_index(drop=True)

                if len(company_df) == 0:
                    print(f"No data for company code {company_code}, skipping...")
                    continue
                ...
                # Process each formula
                for header in self.formula_mappings.keys():
                    result_df = handle_formula(header, company_df, template_df)
                    if not result_df.empty:
                        template_df[header] = result_df[header]
                ...
                # Divide the template into multiple sheets based on Transaction Type
                document_types = company_df['Transaction Type'].unique().tolist()
                ...
                # Add data rows
                for transaction_type, group_data in company_df.groupby('Transaction Type'):
                    template_ws = sheet_dict[transaction_type]
                    template_group = template_df.loc[group_data.index]
                ...
                # Save the workbook with company code in filename
                output_file_path = os.path.join(output_dir, f"poliza_ledger_{company_code}.xlsx")
                template_wb.save(output_file_path)

                generated_files.append({
                    "company_code": company_code,
                    "file_path": output_file_path,
                    "file_name": f"poliza_ledger_{company_code}.xlsx",
                    "rows": len(company_df)
                })

                total_output_rows += len(template_df)
                print(f"Generated: {output_file_path} with {len(company_df)} rows")

            # Prepare result metadata
            result = {
                "success": True,
                "message": f"Successfully generated {len(generated_files)} files for {len(unique_company_codes)} company codes",
